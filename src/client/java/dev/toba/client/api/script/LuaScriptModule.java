...
public class LuaScriptModule extends Module {
    ...
    public static LuaScriptModule load(File file) {
        ...
        try {
            g.loadfile(file.getAbsolutePath()).call();
        } catch (LuaError e) {
            TobaChat.error(file.getName() + ": " + e.getMessage());
        }
    ...
    private static Globals createSandboxedGlobals() {
        ...
        // JseBaseLib (not BaseLib) so loadfile() can read from the filesystem.
        // PackageLib intentionally NOT loaded — scripts must be self-contained (no require())
        g.load(new Bit32Lib());
        g.load(new TableLib());
        g.load(new StringLib());
        g.load(new CoroutineLib());
        g.load(new JseMathLib());
        // NOT loaded: OsLib, IoLib, LuajavaLib, PackageLib
        LoadState.install(g);
        LuaC.install(g);
